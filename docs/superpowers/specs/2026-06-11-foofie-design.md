...
| 3D 渲染 | Three.js (CDN) | 浏览器端渲染，不消耗服务器资源 |
...
生产环境可先使用 CDN 资源以降低构建复杂度，但需要固定版本号。若后续需要更强的离线能力或减少第三方依赖，再将 Tailwind、Three.js 和地图相关资源下载到本地静态目录。
...
```
用户点击「地球」按钮
  → GET /globe 返回 HTML 页面（含 Three.js CDN 引用）
